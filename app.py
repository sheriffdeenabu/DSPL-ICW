...
#Configuring intial page
st.set_page_config(page_title="Sri Lanka Food Suit & Security Dashboard", layout="wide")
...
#Load the Sri Lankan flag with a load spinner to indicate a slight delay if there is to be one
def load_flag_image():
    #Accessed URL from GithHub
...
def main():
    # Sidebar navigation - added Dataset Information option
    page = st.sidebar.radio("Navigation", ["Dashboard", "Project info", "Student Info", "Dataset Info"])

    if page == "About":
        st.title("About the Sri Lanka Food Suit & Security Dashboard")
        st.markdown("""
        ### Dashboard Overview
        ...
        - **Trend Analysis**: Customizable multi-indicator trends over time with the option to select upto three indicators at the same time
        ...
        # Display Sri Lanka flag only on Dashboard page with loading state
        with st.spinner('Loading dashboard...'):
            ...
            if flag:
                ...
                # Perfectly centered header using HTML/CSS
                st.markdown(f"""
                ...
                if not df.empty:
                    ...
                    if categories:
                        ...
                        st.info("Select upto three indicators from the dropdown to visualize and analyse")
